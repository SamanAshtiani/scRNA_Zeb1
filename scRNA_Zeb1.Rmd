---
title: "Zeb1 scRNA-seq"
author: "Saman, based on Lindseynicer tutorial and Wang etal 2020 NatCom"
...
data <- Read10X(data.dir = "~/git/scRNA_Zeb1/data/GSE111429_RAW/")
str(data)
...
# 
DimHeatmap(PG_all, dims = 1:4, cells = 500, balanced = TRUE)
#
...
#now that cluster 8 seems to have high expression of immune markers, we omit the cells in it:
sub_obj <- subset(PG_all, idents = "8", invert = TRUE)
dim(sub_obj)
dim(PG_all)
dim(GetAssayData(PG_all, slot = "data"))

FeaturePlot(sub_obj, features =c('Cd74','Cd72','Cd54') , cols =c('yellow','red'), 
            reduction ='tsne', pt.size = 0.7,ncol = 3, label = T)
...
PG_all@meta.data$RNA_snn_res.0.2
cellRanks$col              <- PG_all@meta.data[order(PG_all@meta.data$RNA_snn_res.0.2),]
cellRanks[1:10,1:10]

PG_all@assays$RNA@scale.data[1:5,1:5]
...
PG.markers.7.filt <- PG.markers.7.filt[!duplicated(PG.markers.7.filt$gene),]
#PG.markers.7.filt <- PG.markers.7[which(PG.markers.7$p_val_adj < 0.05),]
dim(PG.markers.7)
dim(PG.markers.7.filt)

genelist <- PG.markers.7$avg_log2FC
genelist
names(genelist) <-  PG.markers.7$entrezgene_id.x
head(genelist)
...
head(m_df_fgsea)
class(m_df_fgsea) #list

Zhang_BasHi <- read.csv("./data/basal.csv", header = FALSE, fileEncoding="UTF-8-BOM") # add other cell markers from the publication
Zhang_BasHi
Zhang_LumHi <- read.csv("./data/luminal.csv") # add other cell markers from the publication
Zhang_LumHi
...
names(m_df_fgsea[length(m_df_fgsea)])
head(m_df_fgsea[["Zhang_BasHi"]])
...
dim(clusterLabels.pca) #9316  10

connectivity.pca  <- sds@adjacency         # 
dim(connectivity.pca)  #10 10
